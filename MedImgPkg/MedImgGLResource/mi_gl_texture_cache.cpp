...
void GLTextureCache::cache_load(GLenum target, GLTextureBasePtr texture,
...
void GLTextureCache::process_cache() {
  ...
  for (auto it = _cache_list.begin(); it != _cache_list.end(); ++it) {
    ...
    switch (unit->target) {
    case GL_TEXTURE_1D: {
      GLTexture1DPtr cache_tex1d =
          std::dynamic_pointer_cast<GLTexture1D>(unit->texture);
      ...
      if (0 == unit->cache_type) {
        ...
        cache_tex1d->load(unit->internalformat, unit->width, unit->format,
                          unit->type, unit->data, unit->level);
        cache_tex1d->unbind();
      } else if (1 == unit->cache_type) {
        cache_tex1d->bind();
        cache_tex1d->update(unit->xoffset, unit->width, unit->format,
                            unit->type, unit->data, unit->level);
        cache_tex1d->unbind();
      }
      break;
    ...
    case GL_TEXTURE_2D: {
      GLTexture2DPtr cache_tex2d =
          std::dynamic_pointer_cast<GLTexture2D>(unit->texture);
      ...
      if (0 == unit->cache_type) {
        ...
        cache_tex2d->update(unit->xoffset, unit->yoffset, unit->width,
                            unit->height, unit->format, unit->type, unit->data,
                            unit->level);
        cache_tex2d->unbind();
      }
      break;
    ...
    case GL_TEXTURE_3D: {
      GLTexture3DPtr cache_tex3d =
          std::dynamic_pointer_cast<GLTexture3D>(unit->texture);
      ...
      if (0 == unit->cache_type) {
        ...
        cache_tex3d->load(unit->internalformat, unit->width, unit->height,
                          unit->depth, unit->format, unit->type, unit->data,
                          unit->level);
        cache_tex3d->unbind();
      } else if (1 == unit->cache_type) {
        cache_tex3d->bind();
        cache_tex3d->update(unit->xoffset, unit->yoffset, unit->zoffset,
    ...
    case GL_TEXTURE_1D_ARRAY: {
      GLTexture1DArrayPtr cache_tex_array =
          std::dynamic_pointer_cast<GLTexture1DArray>(unit->texture);
      ...
      if (0 == unit->cache_type) {
        ...
        cache_tex_array->load(unit->internalformat, unit->width, unit->height,
                              unit->format, unit->type, unit->data,
                              unit->level);
        cache_tex_array->unbind();
      } else if (1 == unit->cache_type) {
        cache_tex_array->bind();
        cache_tex_array->update(unit->xoffset, unit->yoffset, unit->width,
                                unit->format, unit->type, unit->data,
                                unit->level);
        cache_tex_array->unbind();
      }
    ...
    default: { GLRESOURCE_THROW_EXCEPTION("Invalid texture target in cache!"); }
    }
  }
