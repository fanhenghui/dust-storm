#ifndef MEDIMGRENDERALGO_CAMERA_CALCULATOR_H
#define MEDIMGRENDERALGO_CAMERA_CALCULATOR_H
...
struct PatientAxisInfo {
    ...
    // EG：PatientAxisInfo leftAxisInfo; leftAxisInfo.eVolumeCoord = POSX
    // 说明病人坐标系Left方向最近的体坐标系是正X方向
    CoordinateAxis volume_coord;

    //在该patient方位（head  left  posterior之一） 最靠近 的 volume坐标系轴方向
    //所对应的实际patient方向
    // EG : 同上，leftAxisInfo.vecPatient = (0.9 0 0) , 说明和标准的 (1,0,0)
    // 有偏移
    // 这是为了在world
    // 坐标系下（方向和病人坐标系相同），能够调整相机的位置（未必和病人坐标系轴平行）来观察volume的正方向（为了看扫描位），而非patient坐标系的正方向
...
public:
    ...
    //仅仅改变了度量，从pixel 到 mm ，且在这个坐标系下的Volume bounding
    // box仍然是axis aligned
    ...
    void init_mpr_placement(std::shared_ptr<OrthoCamera> camera,
                            ScanSliceType eScanSliceType,
                            const Point3& ptWorldCenterPoint) const; //ʵʱ��ȡ

    void
    init_mpr_placement(std::shared_ptr<OrthoCamera> camera,
                       ScanSliceType eScanSliceType) const; // Default replacement
    ...
    float convert_thickness_world_to_volume(std::shared_ptr<OrthoCamera>
...
private:
    ...
    OrthoCamera _othogonal_mpr_camera[3];
