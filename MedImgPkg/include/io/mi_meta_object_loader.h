#ifndef MEDIMGIO_META_OBJECTLOADER_H
#define MEDIMGIO_META_OBJECTLOADER_H
...
struct MetaObjectTag {
  ...
  // BinaryData MET_STRING Are the data associated with this object stored at
  // Binary or ASCII
  ...
  // axis.
  // For example : if the three letter code for (column index, row index, slice
  // index is) ILP,
  //                          then the origin is at the superior, right,
  //                          anterior corner of the volume,
  //                          and therefore the axes run from superior to
  //                          inferior, from right to left, from anterior to
  //                          posterior.
  ...
  // Modality MET_STRING
  // One of enum type: MET_MOD_CT, MET_MOD_MR, MET_MOD_US , MET_MOD_OTHER ,
  // MET_MOD_UNKNOWN
  std::string modality;
  ...
  // One of enum type: MET_UCHAR, MET_CHAR , MET_USHORT ,MET_SHORT
  ...
  // 2 A printf-style string followed by the min, max, and step values to be
  // used to pass an argument to the string to create list of file names to be
  // loaded (must be (N-1)D blocks of data per file).
  // 3 LIST [X] – This specifies that starting on the next line is a list of
  // files (one filename per line) in which the data is stored. Each file (by
  // default) contains an (N-1)D block of data. If a second argument is given,
  // its first character must be a number that specifies the dimension of the
  // data in each file. For example ElementDataFile = LIST 2D means that there
...
public:
  IOStatus load(const std::string &info_file,
