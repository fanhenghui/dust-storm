...
float Sampler<T>::sample_2d_linear(float x , float y , unsigned int uiWidth , unsigned int uiHeight , T* data)const
{
    ...
    if (y >= (float)(uiHeight-1) - FLOAT_EPSILON )//TODO 这里必须带==号 否则在诸如y正好为整数的时候会进不去分支
    {
        y = (float)(uiHeight-1);
        uiYAdjust = 0;
    }
    ...
    const float fX00_01= (float)data[uX0Offset]*fX0 + (float)data[uX0Offset +uiXAdjust]*fX1;
    const float fX10_11 = (float)data[uX1Offset]*fX0 + (float)data[uX1Offset +uiXAdjust]*fX1;
...
float Sampler<T>::sample_3d_linear(float x , float y , float z , unsigned int uiWidth , unsigned int uiHeight , unsigned int uiDepth ,T* data)const
{
    ...
    if (z >= (float)(uiDepth-1) - FLOAT_EPSILON )//TODO 这里必须带==号 否则在诸如z正好为整数的时候会进不去分支
    {
        z = (float)(uiDepth-1);
        uiZAdjust = 0;
    }
    ...
    //X direction
    const float fX000_100 = (float)data[uZ0Offset]*fX0 + (float)data[uZ0Offset +uiXAdjust]*fX1;
    const float fX010_110 = (float)data[uZ0Offset + uiYAdjust]*fX0 + (float)data[uZ0Offset +uiYAdjust + uiXAdjust]*fX1;
    const float fX001_101 = (float)data[uZ1Offset]*fX0 + (float)data[uZ1Offset +uiXAdjust]*fX1;
    const float fX011_111 = (float)data[uZ1Offset + uiYAdjust]*fX0 + (float)data[uZ1Offset +uiYAdjust + uiXAdjust]*fX1;
