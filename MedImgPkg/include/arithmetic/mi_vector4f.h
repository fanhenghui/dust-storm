#ifndef MEDIMGARITHMETIC_ARITHMETRIC_VECTOR4F_H
#define MEDIMGARITHMETIC_ARITHMETRIC_VECTOR4F_H
...
public:
  ...
  Vector4f(const float scalar) { _m128 = _mm_set1_ps(scalar); }

  Vector4f(const __m128 vf4) { _m128 = vf4; }
  ...
  inline const Vector3f get_xyz() const { return Vector3f(_m128); }
  ...
  inline bool operator!=(const Vector4f &vec) const {
    ...
    t = _mm_add_ps(_mm_add_ps(vec_splat(t, 0), vec_splat(t, 1)),
                   _mm_add_ps(vec_splat(t, 2), vec_splat(t, 3)));
...
Arithmetic_Export const Vector4f mul_per_elem(const Vector4f &vec0,
                                              const Vector4f &vec1);

// Divide two 4-D vectors per element
// Floating-point behavior matches standard library function divf4.
Arithmetic_Export const Vector4f div_per_elem(const Vector4f &vec0,
                                              const Vector4f &vec1);

// Compute the reciprocal of a 4-D vector per element
// Floating-point behavior matches standard library function recipf4.
Arithmetic_Export const Vector4f recip_per_elem(const Vector4f &vec);

// Compute the absolute value of a 4-D vector per element
Arithmetic_Export const Vector4f abs_per_elem(const Vector4f &vec);

// Maximum of two 4-D vectors per element
Arithmetic_Export const Vector4f max_per_elem(const Vector4f &vec0,
                                              const Vector4f &vec1);

// Minimum of two 4-D vectors per element
Arithmetic_Export const Vector4f min_per_elem(const Vector4f &vec0,
                                              const Vector4f &vec1);

// Maximum element of a 4-D vector
Arithmetic_Export const float max_elem(const Vector4f &vec);

// Minimum element of a 4-D vector
Arithmetic_Export const float min_elem(const Vector4f &vec);

// Compute the sum of all elements of a 4-D vector
Arithmetic_Export const float sum(const Vector4f &vec);

// Compute the dot product of two 4-D vectors
Arithmetic_Export const float dot_product(const Vector4f &vec0,
                                          const Vector4f &vec1);

// Compute the square of the length of a 4-D vector
Arithmetic_Export const float length_sqr(const Vector4f &vec);

// Compute the length of a 4-D vector
Arithmetic_Export const float length(const Vector4f &vec);

// normalize a 4-D vector, result is not accurate enough
// The result is unpredictable when all elements of vec are at or near zero.
Arithmetic_Export const Vector4f normalize_approx(const Vector4f &vec);

// normalize a 4-D vector
// The result is unpredictable when all elements of vec are at or near zero.
Arithmetic_Export const Vector4f normalize(const Vector4f &vec);

// Linear interpolation between two 4-D vectors (scalar data contained in vector
// data type)
// vec0 * (1 - t) + vec1 * t
Arithmetic_Export const Vector4f lerp(const float t, const Vector4f &vec0,
                                      const Vector4f &vec1);
...
Arithmetic_Export const Vector4f slerp(const float t, const Vector4f &unitVec0,
                                       const Vector4f &unitVec1);

// Store x, y, z and w elements of 4-D vector in first four words of a float ptr
Arithmetic_Export void store_xyzw(const Vector4f &vec, float *fptr);
