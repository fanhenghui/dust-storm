#ifndef MEDIMGARITHMETIC_ARITHMETRIC_VECTOR3F_H
#define MEDIMGARITHMETIC_ARITHMETRIC_VECTOR3F_H
...
public:
  Vector3f() { _m128 = _mm_setzero_ps(); }

  Vector3f(const Vector3f &vec) { _m128 = vec._m128; }

  Vector3f(const float _x, const float _y, const float _z) {
    _m128 = _mm_setr_ps(_x, _y, _z, 0.0f);
  }

  Vector3f(const float scalar) { _m128 = _mm_set1_ps(scalar); }

  Vector3f(const __m128 vf4) { _m128 = vf4; }
  ...
  inline void abs() { _m128 = _mm_and_ps(_m128, toM128(0x7fffffff)); }
};
