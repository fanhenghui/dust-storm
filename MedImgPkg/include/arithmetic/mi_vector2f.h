#ifndef MEDIMGARITHMETIC_ARITHMETRIC_VECTOR2F_H
#define MEDIMGARITHMETIC_ARITHMETRIC_VECTOR2F_H
...
public:
  Vector2f() { _m128 = _mm_setzero_ps(); }

  inline Vector2f(const Vector2f &vec) { _m128 = vec._m128; }

  inline Vector2f(const float x, const float y) {
    _m128 = _mm_setr_ps(x, y, 0.0f, 0.0f);
  }

  inline Vector2f(const float scalar) { _m128 = _mm_set1_ps(scalar); }

  inline Vector2f(const __m128 vf4) { _m128 = vf4; }
  ...
  inline const float get_x() const { return _vmathVfGetElement(_m128, 0); }

  inline const float get_y() const { return _vmathVfGetElement(_m128, 1); }
  ...
  inline const __m128 get_128() const { return _m128; }
...
Arithmetic_Export const Vector2f mul_per_elem(const Vector2f &vec0,
                                              const Vector2f &vec1);

// Divide two 2-D vectors per element
// Floating-point behavior matches standard library function divf4.
Arithmetic_Export const Vector2f div_per_elem(const Vector2f &vec0,
                                              const Vector2f &vec1);

// Compute the reciprocal of a 2-D vector per element
// Floating-point behavior matches standard library function recipf4.
Arithmetic_Export const Vector2f recip_per_elem(const Vector2f &vec);

// Compute the absolute value of a 2-D vector per element
Arithmetic_Export const Vector2f abs_per_elem(const Vector2f &vec);

// Maximum of two 2-D vectors per element
Arithmetic_Export const Vector2f max_per_elem(const Vector2f &vec0,
                                              const Vector2f &vec1);

// Minimum of two 2-D vectors per element
Arithmetic_Export const Vector2f min_per_elem(const Vector2f &vec0,
                                              const Vector2f &vec1);

// Maximum element of a 2-D vector
Arithmetic_Export const float max_elem(const Vector2f &vec);

// Minimum element of a 2-D vector
Arithmetic_Export const float min_elem(const Vector2f &vec);

// Compute the sum of all elements of a 2-D vector
Arithmetic_Export const float sum(const Vector2f &vec);

// Compute the dot product of two 2-D vectors
Arithmetic_Export const float dot_product(const Vector2f &vec0,
                                          const Vector2f &vec1);

// Compute the square of the length of a 2-D vector
Arithmetic_Export const float length_sqr(const Vector2f &vec);

// Compute the length of a 2-D vector
Arithmetic_Export const float length(const Vector2f &vec);

// normalize a 2-D vector, result is not accurate enough
// The result is unpredictable when all elements of vec are at or near zero.
Arithmetic_Export const Vector2f normalize_approx(const Vector2f &vec);

// normalize a 2-D vector
// The result is unpredictable when all elements of vec are at or near zero.
Arithmetic_Export const Vector2f normalize(const Vector2f &vec);

// Linear interpolation between two 3-D vectors
// vec0 * (1 - t) + vec1 * t
Arithmetic_Export const Vector2f lerp(const float t, const Vector2f &vec0,
                                      const Vector2f &vec1);
...
Arithmetic_Export const Vector2f slerp(const float t, const Vector2f &unitVec0,
                                       const Vector2f &unitVec1);

// Store x and y elements of 2-D vector in first three words of a float ptr
Arithmetic_Export void store_xy(const Vector2f &vec, float *fptr);
