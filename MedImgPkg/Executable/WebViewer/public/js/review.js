  ...
  function handleImage(
    ...
    function msgHandle(
      ...
      switch (cmdID) {
        case COMMAND_ID_BE_SEND_IMAGE:
          0
          handleImage(
              cellID, tcpBuffer, bufferOffset, dataLen, restDataLen,
              withHeader);
          break;
      ...
      if (tcpPackageEnd == 0) {
        ...
        if (tcpPackageLen - msgRestDataLen == 0) {  // complete last msg
          ...
          msgHandle(
              ...
              false);
        } else if (tcpPackageLen - msgRestDataLen < 0) {  // not complete data
                                                          // yet
        ...
        if (tcpPackageLen <
          ...
          tcpPackageEnd = 1;
          tcpBuffer.copy(
              lastMsgHeader, 0, lastRestHeaderLen, lastRestHeaderLen);
    ...
    window.FE = {
      ...
      init: function(username) {
        ...
        this.socket.emit(
            'login', {userid: this.userid, username: this.username});
    ...
    window.LOGIC = {
      ...
      drawImg: function() {
        ...
        myCtx.putImageData(
            myCanvasImg, 0,
            0);  //把图像数据（从指定的 ImageData 对象）放回画布上
      },
    ...
    // window.FE.init('wr');
        ...
        window.FE.init(document.getElementById('username').innerHTML);
  }
})()