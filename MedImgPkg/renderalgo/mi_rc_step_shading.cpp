...
void RCStepShadingPhong::get_uniform_location()
{
    ...
    if (-1 == _loc_mat_normal ||
        -1 == _loc_spacing ||
        -1 == _loc_light_position ||
        -1 == _loc_ambient_color)
    {
...
void RCStepShadingPhong::set_gpu_parameter()
{
    ...
    const float spacing[3] = {
        static_cast<float>(volume_img->_spacing[0]),
    ...
    glUniform4f(_loc_ambient_color  , ambient_color[0] , ambient_color[1] , ambient_color[2] , ambient_color[3]);

    GLBufferPtr buffer = inner_buffer->get_buffer(RayCasterInnerBuffer::MATERIAL_BUCKET);
    buffer->bind_buffer_base(GL_SHADER_STORAGE_BUFFER , BUFFER_BINDING_MATERIAL_BUCKET);
