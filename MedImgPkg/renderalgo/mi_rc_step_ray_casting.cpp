...
void RCStepRayCastingMIPBase::get_uniform_location() {
    ...
    _loc_pseudo_color_intercept =
        program->get_uniform_location("pseudo_color_intercept");
    _loc_global_wl = program->get_uniform_location("global_wl");

    if (-1 == _loc_pseudo_color || -1 == _loc_pseudo_color_slope ||
            -1 == _loc_pseudo_color_intercept || -1 == _loc_global_wl) {
        RENDERALGO_THROW_EXCEPTION("Get uniform location failed!");
    }
}

GLShaderInfo RCStepRayCastingAverage::get_shader_info() {
    return GLShaderInfo(GL_FRAGMENT_SHADER, S_RC_RAY_CASTING_AVERAGE_FRAG,
...
GLShaderInfo RCStepRayCastingDVR::get_shader_info() {
    return GLShaderInfo(GL_FRAGMENT_SHADER, S_RC_RAY_CASTING_DVR_FRAG,
                        "RCStepRayCastingDVR");
}
