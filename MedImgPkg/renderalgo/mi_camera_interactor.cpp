...
void OrthoCameraInteractor::set_initial_status(
    std::shared_ptr<OrthoCamera> camera) {
...
void OrthoCameraInteractor::reset_camera() { *(_camera.get()) = _camera_init; }
...
void OrthoCameraInteractor::zoom(double scale) { _camera->zoom(scale); }

void OrthoCameraInteractor::zoom(const Point2 &pre_pt, const Point2 &cur_pt,
                                 int width, int height) {
  const double scale = (cur_pt.y - pre_pt.y) / (double)height;
  this->zoom(scale);
}

void OrthoCameraInteractor::pan(const Vector2 &pan) { _camera->pan(pan); }

void OrthoCameraInteractor::pan(const Point2 &pre_pt, const Point2 &cur_pt,
                                int width, int height) {
  ...
  if (mat_p.has_inverse()) {
    ...
    const Point3 pt =
        mat_p.get_inverse().transform(Point3(dx, dy, at_screen.z));

    this->pan(Vector2(pt.x, pt.y));
  }
}

void OrthoCameraInteractor::rotate(const Matrix4 &mat) { _camera->rotate(mat); }

void OrthoCameraInteractor::rotate(const Point2 &pre_pt, const Point2 &cur_pt,
                                   int width, int height) {
  if (pre_pt == cur_pt) {
    return;
  }
  this->rotate(TrackBall::mouse_motion_to_rotation(pre_pt, cur_pt, width,
                                                   height, Point2::S_ZERO_POINT)
                   .to_matrix());
}
