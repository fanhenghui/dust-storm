...
int CameraCalculator::get_page_maximum(ScanSliceType type) const {
  ...
  default: { RENDERALGO_THROW_EXCEPTION("Cant get oblique maximun page!"); }
  }
}
...
int CameraCalculator::get_default_page(ScanSliceType type) const {
  ...
  default: { RENDERALGO_THROW_EXCEPTION("Cant get oblique default page!"); }
  }
}
...
void CameraCalculator::caluculate_orthogonal_mpr_placement_i() {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    default: { RENDERALGO_THROW_EXCEPTION("Invalid scan slice type!"); }
    }
