...
void RayCastingCPU::render_entry_exit_points_i( int test_code)
{
    ...
    if (1 == test_code)
    {
        for (int i = 0 ; i < pixel_sum ; ++i)
        {
            ...
            _canvas_array[i] = RGBAUnit::norm_to_integer(start_point._m[0] , start_point._m[1] , start_point._m[2]);
        }
    }
    else
    {
        for (int i = 0 ; i < pixel_sum ; ++i)
        {
            ...
            _canvas_array[i] = RGBAUnit::norm_to_integer(end_point._m[0] , end_point._m[1] , end_point._m[2]);
        }
    }
    ...
    for (int idx = 0; idx<pixel_sum  ; ++idx)
    {
        ...
        for (int i = 0 ; i < step ; ++i)
        {
            ...
            sample_value = sampler.sample_3d_nearst(
                sample_pos._m[0] , sample_pos._m[1] , sample_pos._m[2] , 
                _dim[0], _dim[1], _dim[2] , _mask_data_array);
        ...
        current_rgba.g = current_color._m[1] > 255.0f ? 255 : static_cast<unsigned char>(current_color._m[1]);
        current_rgba.b = current_color._m[2] > 255.0f ? 255 : static_cast<unsigned char>(current_color._m[2]);
        current_rgba.a = 255;
