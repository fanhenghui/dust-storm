...
ColorTransFunc::ColorTransFunc(int width /*= 512 */)
    : _width(width), _interpolation_type(RGB), _input_type(RGB),
      _is_dirty(false) {}

ColorTransFunc::~ColorTransFunc() {}

void ColorTransFunc::set_name(const std::string &lut_name) { _name = lut_name; }
...
void ColorTransFunc::set_color_type(ColorType input_type,
                                    ColorType interpolation_type) {
  _interpolation_type = interpolation_type;
  _input_type = input_type;
  _is_dirty = true;
}

void ColorTransFunc::add_rgb_point(float real_value, float r, float g,
                                   float b) {
  r = r > 255.0f ? 255.0f : r;
  r = r < 0.0f ? 0.0f : r;
  ...
  _tp_points.push_back(ColorTFPoint(real_value, r, g, b));
  _is_dirty = true;
}

void ColorTransFunc::add_hsv_point(float real_value, float h, float s,
                                   float v) {
  h = h > 360.0f ? 360.0f : h;
  h = h < 0.0f ? 0.0f : h;
  ...
  _tp_points.push_back(ColorTFPoint(real_value, h, s, v));
  _is_dirty = true;
}
...
void ColorTransFunc::get_point_list(std::vector<ColorTFPoint> &result_list) {
  if (_is_dirty) // Point changed
  {
    ...
    for (size_t i = 0; i < tp_point_size; ++i) {
      for (size_t j = 0; j < tp_point_size - 1 - i; ++j) {
        if (_tp_points[j].v > _tp_points[j + 1].v) {
          ColorTFPoint temp(_tp_points[j].v, _tp_points[j].x, _tp_points[j].y,
                            _tp_points[j].z);
    ...
    for (size_t i = 0; i < tp_point_size - 1; ++i) {
      ...
      float step_x =
          (_tp_points[i + 1].x - _tp_points[i].x) / static_cast<float>(gap);
      float step_y =
          (_tp_points[i + 1].y - _tp_points[i].y) / static_cast<float>(gap);
      float step_z =
          (_tp_points[i + 1].z - _tp_points[i].z) / static_cast<float>(gap);
      ...
      for (int j = 0; j < gap; ++j) {
        ...
        begin_value += 1.0f;
        _result_points[idx++] =
            (ColorTFPoint(begin_value, begin_x, begin_y, begin_z));
      }
    }
...
int ColorTransFunc::get_width() const { return _width; }
...
ColorTFPoint ColorTransFunc::rgb_to_hsv(const ColorTFPoint &rgb) {
  ...
  return ColorTFPoint(rgb.v, h * 360.0f, s, v);
}
