...
IOStatus TransferFuncLoader::load_pseudo_color(const std::string& xml , std::shared_ptr<ColorTransFunc>& color)
{
    ...
    for (auto it = points.begin() ; it != points.end() ; ++it)
    {
        ...
        if (idx_node.empty() || r_node.empty() || g_node.empty() || b_node.empty())
        {
            return IO_DATA_DAMAGE;
        }
...
IOStatus TransferFuncLoader::load_color_opacity(
    ...
    std::shared_ptr<ColorTransFunc>& color , std::shared_ptr<OpacityTransFunc>& opacity ,
    float &ww , float &wl,
    RGBAUnit& background,
    Material& material)
{
    ...
    const std::string color_inter = color_node.attribute("Interpolation").as_string();
    if (color_inter == "RGB")
    {
        color->set_color_type(RGB , RGB);
    }
    ...
    if (diffuse_r.empty() || diffuse_g.empty() || diffuse_b.empty() || diffuse_factor.empty())
    {
        return IO_DATA_DAMAGE;
    }
    ...
    if (specular_r.empty() || specular_g.empty() || specular_b.empty() || specular_factor.empty() || specular_shininess.empty())
    {
        return IO_DATA_DAMAGE;
    }
