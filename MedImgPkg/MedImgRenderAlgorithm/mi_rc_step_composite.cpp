...
void RCStepCompositeDVR::set_gpu_parameter()
{
    ...
    GLBufferPtr buffer_wl = inner_buffer->get_buffer(RayCasterInnerBuffer::WINDOW_LEVEL_BUCKET);
    buffer_wl->bind_buffer_base(GL_SHADER_STORAGE_BUFFER , BUFFER_BINDING_WINDOW_LEVEL_BUCKET);

    GLTexture1DArrayPtr color_opacity_tex_array = ray_caster->get_color_opacity_texture_array();
    ...
    const double min_spacing = (std::min)((std::min)(volume_img->_spacing[0] , volume_img->_spacing[1]) , volume_img->_spacing[2]);
...
{
    GLProgramPtr program = _gl_program.lock();
    _loc_color_opacity_array = program->get_uniform_location("color_opacity_array");
    _loc_color_opacity_texture_shift = program->get_uniform_location("color_opacity_texture_shift");
