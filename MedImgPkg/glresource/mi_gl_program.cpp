...
unsigned int GLProgram::get_id() const { return _program_id; }
...
void GLProgram::compile() {
  ...
  for (auto it = _shaders.begin(); it != _shaders.end(); ++it) {
    ...
    if (!compile_status) {
      ...
      for (auto it_delete = _shaders.begin(); it_delete != _shaders.end();
           ++it_delete) {
  ...
  if (!link_status) {
    ...
    std::unique_ptr<GLchar[]> log_context(new GLchar[log_length + 1]);
    glGetProgramInfoLog(_program_id, log_length, &log_length,
                        log_context.get());
    log_context[log_length] = '\0';
...
void GLProgram::bind() { glUseProgram(_program_id); }
