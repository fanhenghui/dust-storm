...
GLTexture1D::GLTexture1D(UIDType uid)
    : GLTextureBase(uid), _width(0), _format(GL_RGBA),
...
void GLTexture1D::bind() { glBindTexture(GL_TEXTURE_1D, _texture_id); }

void GLTexture1D::unbind() { glBindTexture(GL_TEXTURE_1D, 0); }

void GLTexture1D::load(GLint internalformat, GLsizei width, GLenum format,
                       GLenum type, const void *data, GLint level /*= 0*/) {
  glTexImage1D(GL_TEXTURE_1D, level, internalformat, width, 0, format, type,
               data);
  _width = width;
  _format = format;
...
GLsizei GLTexture1D::get_width() { return _width; }

GLenum GLTexture1D::get_format() { return _format; }
