...
bool PACSCommunicator::initialize(const char * self_AE_title, const unsigned short self_port, const char * serive_ip_address, const unsigned short serive_port, const char * service_AE_title)
{
    if (!this->_initialized)
    {
        ...
        if (!this->_scu->createAssociation(serive_ip_address, serive_port, service_AE_title))
        {
...
bool PACSCommunicator::initialize(const char * configure_file_path)
{
    ...
    {
        std::cout << "Cannot open the specified file :(\n try another initializer \n";
        return false;
    }
    else
    {
        ...
        return this->initialize(
            client_title.c_str(), client_port,
            server_host.c_str(), server_port, server_title.c_str());
    }
}
...
const std::string PACSCommunicator::fetch_dicom(const WorkListInfo& item)
{
    ...
    OFString directory = OFString((_cache_path + "/" + item.GetSeriesInsUID()).c_str());
...
#else
        std::string cmd = std::string("rm -r ") + std::string("\"") +std::string(directory.c_str()) + std::string("\"");
#endif
