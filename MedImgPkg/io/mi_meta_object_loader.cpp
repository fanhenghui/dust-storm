...
IOStatus
MetaObjectLoader::load(const std::string& info_file,
...
IOStatus MetaObjectLoader::construct_meta_object_tag(
    ...
    while (std::getline(in, sLine)) {
        ...
        if (tag == META_COMMENT) {
            ...
            ss >> meta_obj_tag->spacing[0] >> meta_obj_tag->spacing[1] >>
               meta_obj_tag->spacing[2];
        } else if (tag == META_DIM_SIZE) {
            ss >> meta_obj_tag->dim_size[0] >> meta_obj_tag->dim_size[1] >>
               meta_obj_tag->dim_size[2];
    ...
    // Check meta obj tags
    if (meta_obj_tag->n_dims != 3 || meta_obj_tag->dim_size[0] == 0 ||
            meta_obj_tag->dim_size[1] == 0 || meta_obj_tag->dim_size[2] == 0 ||
...
IOStatus MetaObjectLoader::construct_data_header(
    ...
    img_data_header->patient_id =
        str_num_converter.to_string(meta_obj_tag->parent_id);
    ...
    if (postfix_sub != -1 &&
        ...
        img_data_header->series_uid =
            meta_obj_tag->element_data_file.substr(0, postfix_sub);
    }
...
IOStatus MetaObjectLoader::construct_image_data(
    ...
    const std::string file1 =
        info_file.substr(0, file_dir_sub + 1) + meta_obj_tag->element_data_file;
    ...
    if (meta_obj_tag->is_compressed_data) {
        ...
        IOStatus status = ZLibUtils::decompress(
                              raw_file, (char*)img_data->get_pixel_pointer(), img_size);
        return status;
    } else {
        img_data->mem_allocate();
        const unsigned int img_size = img_data->get_data_size();
