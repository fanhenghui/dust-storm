...
MIDcmSCU::MIDcmSCU(const char *self_AE_title)
    : _association_ready(false), _p_work_list(nullptr) {
  // set AE titles
  this->setAETitle(self_AE_title);
}

bool MIDcmSCU::createAssociation(const char *serive_ip_address,
  ...
  if (!this->_association_ready) {
    ...
    this->addPresentationContext(UID_FINDStudyRootQueryRetrieveInformationModel,
                                 ts);
    this->addPresentationContext(UID_MOVEStudyRootQueryRetrieveInformationModel,
                                 ts);
    ...
    */
    ...
    if (result.bad()) {
      DCMNET_ERROR(
          "Could not process C-ECHO with the server: " << result.text());
      return this->_association_ready;
    }
...
bool MIDcmSCU::search(DcmDataset &query_keys) {
  T_ASC_PresentationContextID presID =
      findUncompressedPC(UID_FINDStudyRootQueryRetrieveInformationModel, *this);
  if (presID == 0) {
    DCMNET_ERROR(
        "There is no uncompressed presentation context for Study Root FIND");
    return false;
  }
  OFCondition result = this->sendFINDRequest(
      presID, &query_keys, NULL /* we are not interested into responses*/);
  if (result.bad())
    return false;
  else
    // DCMNET_INFO("There are " << findResponses.size() << " studies
    // available");
    return true;
}
...
bool MIDcmSCU::fetch(const char *dst_AE_title, const WorkListInfo &which_one) {
  ...
  queryKeys.putAndInsertString(DCM_QueryRetrieveLevel, "SERIES");
  queryKeys.putAndInsertString(DCM_StudyInstanceUID,
                               which_one.GetStudyInsUID().c_str());
  queryKeys.putAndInsertString(DCM_SeriesInstanceUID,
                               which_one.GetSeriesInsUID().c_str());
...
bool MIDcmSCU::fetch(const char *dst_AE_title, DcmDataset &query_keys) {
  ...
  T_ASC_PresentationContextID presID =
      findUncompressedPC(UID_MOVEStudyRootQueryRetrieveInformationModel, *this);
  if (presID == 0) {
    DCMNET_ERROR(
        "There is no uncompressed presentation context for Study Root MOVE");
    return false;
  }

  // start an additional storage server  (e.g., storescp.exe) before calling
  // sendMOVERequest

  OFCondition result =
      this->sendMOVERequest(presID, dst_AE_title, &query_keys,
                            NULL /* we are not interested into responses*/);
  if (result.bad()) {
    return false;
  } else
    return true;
...
OFCondition
MIDcmSCU::handleFINDResponse(const T_ASC_PresentationContextID presID,
                             QRResponse *response,
...
void MIDcmSCU::addFindResult2List(QRResponse *response,
  ...
  if (response->m_dataset != nullptr) {
    ...
    responseIdentifiers->findAndGetString(DCM_SpecificCharacterSet,
                                          cCharacterSet);
    ...
    responseIdentifiers->findAndGetString(DCM_PatientBirthDate,
                                          cPatientBirthday);
    ...
    responseIdentifiers->findAndGetString(DCM_StudyDescription,
                                          cStudyDescription);
    responseIdentifiers->findAndGetString(DCM_ReferringPhysicianName,
                                          cReferringPhysician);
    responseIdentifiers->findAndGetString(DCM_RequestingPhysician,
                                          cRequestingPhysician);
    ...
    responseIdentifiers->findAndGetString(DCM_ScheduledStationAETitle,
                                          cScheduledAETitle, true);
    ...
    responseIdentifiers->findAndGetString(DCM_RequestedProcedureID,
                                          cRequestedProcedureID, true);
    responseIdentifiers->findAndGetString(DCM_ScheduledProcedureStepID,
                                          cScheduledProStepID, true);
    responseIdentifiers->findAndGetString(DCM_ScheduledProcedureStepDescription,
                                          cScheduledProStepDes, true);
    responseIdentifiers->findAndGetString(DCM_ScheduledProcedureStepStartDate,
                                          cScheduledProStepStartDate, true);
    responseIdentifiers->findAndGetString(DCM_ScheduledProcedureStepStartTime,
                                          cScheduledProStepStartTime, true);
    responseIdentifiers->findAndGetString(DCM_RequestedProcedureDescription,
    ...
    if (NULL != cScheduledProStepStartDate) {
      ...
      tempWorklistInfo.SetScheduledProcedureStepStartDate(
          sScheduledProStepStartDate);
    }

    if (NULL != cScheduledProStepStartTime) {
      std::string sScheduledProStepStartTime(cScheduledProStepStartTime);
      tempWorklistInfo.SetScheduledProcedureStepStartTime(
          sScheduledProStepStartTime);
    }
