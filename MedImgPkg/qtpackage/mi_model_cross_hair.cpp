...
void CrosshairModel::set_mpr_scene(const ScanSliceType(&scan_type)[3] ,
...
void CrosshairModel::get_cross_line(const MPRScenePtr& target_mpr_scene, Line2D(&lines)[2] ,
                                    RGBUnit(&color)[2]) {
...
bool CrosshairModel::locate(const std::shared_ptr<MPRScene>& target_mpr_scene ,
                            const Point2& pt_dc) {
    ...
    _location_discrete_w = mat_v2w.transform(Point3((double)((int)ptV.x) , (double)((int)ptV.y) ,
                           (double)((int)ptV.z)));
