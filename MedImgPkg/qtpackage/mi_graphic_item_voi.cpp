...
void GraphicItemVOI::update(std::vector<QGraphicsItem*>& to_be_add ,
    ...
    try {
        ...
        for (size_t i = 0 ; i < circle_center.size() ; ++i , ++item_sphere , ++item_info , ++item_line) {
            ...
            if (!(*item_sphere)->is_frezze()) {
                ...
                (*item_sphere)->set_sphere(QPointF(static_cast<float>(circle_center[i].x) ,
                                                   static_cast<float>(circle_center[i].y)) , radiuses[i]);
                (*item_sphere)->show();
            }
            ...
            //line
            (*item_line)->setLine(QLineF(sphere_pos + QPointF(sphere_w , 0), info_pos + QPointF(0 ,
                                         10.0f * 4.0f)));
            (*item_line)->show();
        }
...
GraphicsSphereItem::GraphicsSphereItem(QGraphicsItem* parent /*= 0 */,
                                       QGraphicsScene* scene /*= 0*/): _id(-1), _is_frezze(false) {
...
void GraphicsSphereItem::update_circle_center_i() {
    _pre_center = QPointF(this->pos().x() + this->rect().width() * 0.5f ,
                          this->pos().y() + this->rect().height() * 0.5f);
...
void GraphicsSphereItem::update_sphere_center_i(int code_id /*= 0*/) {
    ...
    Point2 cur_circle_center_ndc = ArithmeticUtils::dc_to_ndc(Point2(_pre_center.x() ,
                                   _pre_center.y())  , width , height);
    Point3 cur_circle_center = mat_vp_inv.transform(Point3(cur_circle_center_ndc.x,
                               cur_circle_center_ndc.y , 0.0));
...
GraphicsTextItem::GraphicsTextItem(GraphicsLineItem* line_item): _is_grabbed(false) {
    connect(this , SIGNAL(position_changed(QPointF)) , line_item ,
            SLOT(slot_info_position_changed(QPointF)));
}
