...
ImgSeqGen::ImgSeqGen()
    : _width(0), _height(0), _deep(0), _min(-65535.0f), _max(65535.0f) {}

ImgSeqGen::~ImgSeqGen() {}

void ImgSeqGen::set_raw_data(const std::string &path, int width, int height,
                             int deep) {
...
unsigned char *ImgSeqGen::gen_img(int slice) {
  ...
  if (_data && slice > 0 && slice < _deep - 1) {
    unsigned short *img_data =
        (unsigned short *)_data.get() + slice * _width * _height;
    const float ww = 2352.0f;
    const float wl = 1179.0f;
    const float gray_min = wl - ww * 0.5f;
    for (int i = 0; i < _width * _height; ++i) {
      float gray =
          (static_cast<float>(ntohs(img_data[i])) - gray_min) / ww * 255.0f;
