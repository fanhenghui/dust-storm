...
void DICOMAnonymizationDlg::slot_press_btn_browse_path_i() {
    QString file_folder = QFileDialog::getExistingDirectory(this , tr("Export path dialog") ,
...
void DICOMAnonymizationDlg::slot_press_btn_export_i() {
    ...
    for (int i = 0 ; i < _dicom_series_files.size() ; ++i) {
        ...
        std::string file_name = custom_export_path + std::string("\\") + custom_prefix + file_nake +
                                custom_postfix;
...
void DICOMAnonymizationDlg::set_progress_model(std::shared_ptr<medical_imaging::ProgressModel>
        model) {
