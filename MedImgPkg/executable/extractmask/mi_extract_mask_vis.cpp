...
extern int load_dicom_series(std::vector<std::string>& files ,
                             std::shared_ptr<ImageDataHeader>& header, std::shared_ptr<ImageData>& img);

extern int get_nodule_set(const std::string& xml_file, std::vector<std::vector<Nodule>>& nodules ,
                          std::string& series_uid);

extern int resample_z(std::vector<std::vector<Nodule>>& nodules ,
...
extern void get_same_region_nodules(std::vector <std::vector<Nodule>>::iterator reader ,
                                    Nodule& target , std::vector <std::vector<Nodule>>& nodules, std::vector<Nodule*>& same_nodules,
                                    std::vector<int>& reader_id ,  float region_percent);

typedef ScanLineAnalysis<unsigned char>::Pt2 PT2;
extern void scan_contour_to_mask(std::vector<Point3>& pts , std::shared_ptr<ImageData> mask,
                                 unsigned char label);

extern int contour_to_mask(std::vector <std::vector<Nodule>>& nodules ,
                           std::shared_ptr<ImageData> mask, float same_nodule_precent , int confidence, int pixel_confidence ,
                           int setlogic);

extern int browse_root_xml(const std::string& root , std::vector<std::string>& xml_files);

extern int browse_root_dcm(const std::string& root,
                           std::map<std::string, std::vector<std::string>>& dcm_files);

extern int extract_mask(int argc , char* argv[] , std::shared_ptr<ImageData>& last_img ,
