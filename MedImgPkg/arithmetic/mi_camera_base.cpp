...
CameraBase::CameraBase()
    : _eye(Point3::S_ZERO_POINT), _at(Point3::S_ZERO_POINT),
      _up(Vector3::S_ZERO_VECTOR), _mat_view(Matrix4::S_IDENTITY_MATRIX),
...
Point3 CameraBase::get_eye() const { return _eye; }

Point3 CameraBase::get_look_at() const { return _at; }

Vector3 CameraBase::get_up_direction() const { return _up; }
...
void CameraBase::rotate(const Quat4 &quat) { rotate(quat.to_matrix()); }
...
void CameraBase::rotate(const Matrix4 &mat) {
  ...
  if (!mat_pre_view.has_inverse()) {
    ...
    mat_pre_view.prepend(
        make_translate(Point3(0, 0, 0) - mat_pre_view.transform(_at)));
