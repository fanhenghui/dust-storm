#include "mi_cmd_handler_be_db_send_dicom.h"
...
CmdHandlerBE_DBSendDICOM::CmdHandlerBE_DBSendDICOM(std::shared_ptr<AppController> controller):
...
CmdHandlerBE_DBSendDICOM::~CmdHandlerBE_DBSendDICOM() {
    _th_cache_db.join();
    //remove package
...
int CmdHandlerBE_DBSendDICOM::handle_command(const IPCDataHeader& ipcheader , char* buffer) {
    try {
        ...
        if (!_th_running) {
            _th_cache_db = boost::thread(boost::bind(&CmdHandlerBE_DBSendDICOM::update_cache_db_i, this));
            _th_running = true;
            _cur_recv_slice = 0;
...
void CmdHandlerBE_DBSendDICOM::update_cache_db_i() {
    while(true) {
