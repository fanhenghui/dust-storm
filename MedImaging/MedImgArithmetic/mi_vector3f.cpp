...
{
    __m128 t = _mm_max_ps( _mm_max_ps( vec_splat( vec._m128, 0 ), vec_splat( vec._m128, 1 ) ), vec_splat( vec._m128, 2 ) );
...
{
    __m128 t = _mm_min_ps( _mm_min_ps( vec_splat( vec._m128, 0 ), vec_splat( vec._m128, 1 ) ), vec_splat( vec._m128, 2 ) );
    return _vmathVfGetElement(t , 0);
}
...
{
    __m128 t =  _mm_add_ps( _mm_add_ps( vec_splat( vec._m128, 0 ), vec_splat( vec._m128, 1 ) ), vec_splat( vec._m128, 2 ) );
    return _vmathVfGetElement(t , 0);
}
...
{
    return Vector3f( _mm_mul_ps( vec._m128, _mm_rsqrt_ps( _vmathVfDot3( vec._m128, vec._m128 ) ) ) );
}

 
    const Vector3f normalize( const Vector3f &vec )
{
    return Vector3f( _mm_mul_ps( vec._m128, newtonrapson_rsqrt4( _vmathVfDot3( vec._m128, vec._m128 ) ) ) );
}
...
#define _MCSF_3D_SLERP_TOL 0.999f
    ...
    cosAngle = _vmathVfDot3( unitVec0._m128, unitVec1._m128 );
    ...
    return Vector3f( vec_madd( unitVec0._m128, scale0, _mm_mul_ps( unitVec1._m128, scale1 ) ) );
}
