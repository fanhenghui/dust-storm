...
IOStatus ZLibUtils::compress(const std::string& src_path , const std::string& dst_path)
{
...
IOStatus ZLibUtils::decompress(const std::string& src_path , const std::string& dst_path)
{
    std::ifstream src_file(src_path.c_str() , std::ios::in | std::ios::binary);
    std::ofstream dst_file(dst_path.c_str() , std::ios::out | std::ios::binary);
    if (!src_file.is_open() || !dst_file.is_open())
    ...
    do
    {
        ...
        do 
        {
            ...
            if (ret == Z_NEED_DICT || ret == Z_DATA_ERROR || ret == Z_MEM_ERROR)//解压到数据末尾不是Z_OK
            {
                src_file.close();
                dst_file.close();
                inflateEnd(&strm);
...
IOStatus ZLibUtils::decompress(const std::string& src_path , char* dst_buffer , unsigned int out_size)
{
    std::ifstream src_file(src_path.c_str() , std::ios::in | std::ios::binary);
    if (!src_file.is_open())
    {
