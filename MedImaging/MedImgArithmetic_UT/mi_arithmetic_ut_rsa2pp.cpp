#include <iostream>
#include "MedImgArithmetic/mi_rsa_utils.h"
#include <time.h>

using namespace medical_imaging;
namespace
{
    static const char* S_N = "";
    static const char* S_E = "010001";
    static const char* S_D = "";
    static const char* S_P = "";
    static const char* S_Q = "";
    static const char* S_DP = "";
    static const char* S_DQ = "";
    static const char* S_QP = "";


    int get_rand_entrypt_id()
    {
        static int tick = 0;
        tick += (int(clock()) % 13 + 1);

        int id = 0;
        if (tick > 51)
        {
            id = 1;
            tick = 0;
        }
        else
        {
            id = 0;
        }

        return id;
    }
}

void UT_RSA2()
{

    int loop = 0;
    int got = 0;
    while(loop < 1000)
    {
        got += get_rand_entrypt_id();
        ++loop;
    }

    std::cout << got << std::endl;
    return;

    int status = 0;
    mbedtls_rsa_context rsa;

    RSAUtils rsa_tools;

    //status = rsa_tools.gen_key(rsa);

    //if(status != 0)
    //{
    //    std::cout << "Generate key failed!\n";
    //}

    //rsa_tools.key_to_file(rsa , "D:/temp/rsa_pub.txt" , "D:/temp/rsa_priv.txt");

    status = rsa_tools.to_rsa_context(S_N , S_E , S_D , S_P , S_Q , S_DP , S_DQ , S_QP , rsa);
    if(status != 0)
    {
        std::cout << "to rsa context failed!\n";
    }

    const int num = 24;
    double pos[num]; 
    for (int i = 0 ; i < num ; ++i)
    {
        pos[i] = (double)i + (double)i*0.1 + (double)i*0.01 + (double)i*0.001+ (double)i*0.0001+ (double)i*0.00001 + (double)i*0.000001+ (double)i*0.0000001;
    }

    unsigned char entrypt_output_pos[512];
    memset(entrypt_output_pos , 0 , sizeof(entrypt_output_pos));

    status = rsa_tools.entrypt(rsa , sizeof(pos),(unsigned char*)(pos) , entrypt_output_pos);
    if(status != 0)
    {
        std::cout << "entrypt failed!\n";
    }

    unsigned char detrypt_output_pos[1024];

    memset(detrypt_output_pos , 0 , sizeof(detrypt_output_pos));


    status = rsa_tools.detrypt(rsa ,  512 , entrypt_output_pos , detrypt_output_pos);
    if(status != 0)
    {
        std::cout << "detrypt failed!\n";
    }

    double* out_array = (double*)(detrypt_output_pos);

    for (int i = 0 ; i< 24 ; ++i)
    {
        if (abs(out_array[i] - pos[i]) > DOUBLE_EPSILON)
        {
            std::cout << "error\n";
        }
        std::cout << out_array[i] << "\n";
    }
    /*std::cout << out_array[0] << "\n";
    std::cout << out_array[1] << "\n";
    std::cout << out_array[2] << "\n";*/
    std::cout << "Done \n";

}